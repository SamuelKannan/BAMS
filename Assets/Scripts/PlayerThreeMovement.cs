...
public class PlayerThreeMovement : MonoBehaviour
{
    ...
    void ProcessInputs()
    {
        dirPlayerX = Input.GetAxisRaw("Player3_Horizontal");
        dirPlayerY = Input.GetAxisRaw("Player3_Vertical");
    ...
    void Timer()
    {
        if (isReady == false)
        {
            ...
            Debug.Log("Player3 Time Elapsed: " + timeElasped);
            if (timeElasped <= 0.0f)
            {
                isReady = true;
    ...
    {
        if (collision.gameObject.tag == "Player1" || collision.gameObject.tag == "Player2" || collision.gameObject.tag == "Player4")
        {
    ...
    void Attack()
    {
        if (Input.GetKeyDown("o") && inContact == true && isReady == true)
        {
